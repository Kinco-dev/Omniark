...
contract Omniark is ERC20, Ownable {
    ...
    modifier onlyPresale() {
        require(_presaleAddresses[msg.sender] , "OARK: caller is not one of the presale address");
        _;
    }
    ...
    constructor() ERC20("OMNIARK", "OARK") {
    ...
    function excludeFromReward(address account, bool excluded) public onlyOwner {
        require((account != address(this) && account != dexPair) || excluded, "OARK: Main pair and contract's addresses cannot be included from rewards");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "OARK: Account has already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;
        emit ExcludeFromFees(account, excluded);
    }

    function excludeFromMaxSellLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxSellLimit[account] != excluded, "OARK: Account has already the value of 'excluded'");
        _isExcludedFromMaxSellLimit[account] = excluded;
        emit ExcludeFromMaxSellLimit(account, excluded);
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != dexPair, "OARK: The main pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "OARK: Automated market maker pair is already set to that value");
    ...
    function setBuyFees(uint8 rewardFee, uint8 liquidityFee, uint8 marketingFee) external onlyOwner {
        ...
        require(newTotalBuyFees <= 10 && newTotalBuyFees >=0,"OARK: Total fees must be between 0 and 10");
    ...
    function setSellFees(uint8 rewardFee,uint8 liquidityFee,uint8 marketingFee) external onlyOwner {
        ...
        require(newTotalSellFees <= 20 && newTotalSellFees >=0, "OARK: Total fees must be between 0 and 20");
    ...
    function setTransferFees(uint8 rewardFee,uint8 liquidityFee,uint8 marketingFee) external onlyOwner {
        ...
        require(newTotalTransferFees <= 10 && marketingFee >=0, "OARK: Total fees must be between 0 and 10");
    ...
    function setMaxSellLimit(uint256 amount) external onlyOwner {
        require(amount >= totalSupply()/1000 / 10**9, "OARK: Max sell limit must be greater or equals than 0.1% of the total supply");
        maxSellLimit = amount *10**9;
        emit MaxSellLimitUpdated(amount);
    }

    function setSwapThreshold(uint256 amount) external onlyOwner {
        require(amount >= 1 && amount <= totalSupply()/50 / 10**9, "OARK: Amount must be greater or equals than 1 and lower or equals than 2% of the total supply");
        swapThreshold = amount *10**9;
        emit SwapThresholdUpdated(amount);
    }

    function setLiquidityWallet(address newWallet) public onlyOwner {
        require(newWallet != liquidityWallet, "OARK: The liquidity wallet has already this address");
        emit LiquidityWalletUpdated(newWallet, liquidityWallet);
        liquidityWallet = newWallet;
    }
    function setMarketingWallet(address payable newWallet) external onlyOwner {
        require(newWallet != marketingWallet, "OARK: The marketing wallet has already this address");
        emit MarketingWalletUpdated(newWallet,marketingWallet);
        marketingWallet = newWallet;
    }

    function enableTrading() external onlyOwner {
        require(!tradingIsEnabled,"OARK: Trading is already enabled");
        tradingIsEnabled = true;
        emit TradingEnabled();
    }

    function addPresaleAccount(address account) public onlyOwner {
        require(!_presaleAddresses[account],"OARK: This account is already added");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "OARK: Transfer from the zero address");
        require(to != address(0), "OARK: Transfer to the zero address");
        require(amount >= 0, "OARK: Transfer amount must be greater or equals to zero");

        bool isBuyTransfer = automatedMarketMakerPairs[from];
        bool isSellTransfer = automatedMarketMakerPairs[to];

        if(!tradingIsEnabled) {
            require(_presaleAddresses[from], "OARK: This account is not allowed to send tokens before trading is enabled");
        }

        if(tradingIsEnabled && !_isLiquefying && isSellTransfer && !_isExcludedFromMaxSellLimit[from] && from != address(dexRouter)) {
            require(amount <= maxSellLimit, "OARK: Amount exceeds the maxSellLimit");
        }
    ...
    function distributeTokensManually() external payable onlyOwner {     
        require(tradingIsEnabled,"OARK: Trading must be enabled to distribute tokens");
    ...
    function batchTokensTransfer(address[] calldata _accounts, uint256[] calldata _amounts) external onlyPresale {
        require(_accounts.length <= 200, "OARK: 200 addresses maximum");
        require(_accounts.length == _amounts.length, "OARK: Account array must have the same size as the amount array");
        require(_isExcludedFromReward[_msgSender()], "OARK: Owner must be excluded from reward");
    ...
    function getStuckBNBs(address payable to) external onlyOwner {
        require(address(this).balance > 0, "OARK: There are no BNBs in the contract");
        to.transfer(address(this).balance);
    } 

    function getStuckTokens(address payable to, address token, uint256 amount) external onlyOwner {
        require(IERC20(token).balanceOf(address(this)) > 0, "OARK: There are no tokens in the contract");
        require(token != address(this),"OARK: OARK tokens cannot be got from the contract");
        IERC20(token).transfer(to,amount);
    }


    function setDistributorSettings(uint256 newGas) external onlyOwner {
        require(newGas >= 100_000 && newGas <= 900_000, "OARK: Gas must be between 100,000 and 900,000");
        require(newGas != distributorGas, "OARK: DistributorGas is already this value");
